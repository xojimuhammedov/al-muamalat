...
const Course = () => {
    ...
    const aboutData = courseData?.find((item) => item?.id === certificate)
    return (
        <>
            <Box p={'36px 0'}>
                <Box className='container'>
                    <Heading {...css.title}>{t("O’quv kurslari")}</Heading>
                    <SimpleGrid mt={'36px'} columns={{ base: 1, md: 2, lg: 3 }} gap={{ base: "12px", md: '24px' }}>
                        {courseData?.map((item) => (
                            <Box {...css.item}>
                                <Image src={item.image} {...css.image} />
                                <Heading noOfLines={2} {...css.name}>{item?.[`title_${i18n?.language}`]}</Heading>
                                <Text {...css.subtext}
                                    noOfLines={3}
